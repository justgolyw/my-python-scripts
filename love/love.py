...
class MessageBox(QtWidgets.QWidget):#继承自父类QtWidgets.QWidget
    ...
    def __init__(self):
        ...
        self.setGeometry(300,300,600,600)  #setGeometry()方法完成两个功能--设置窗口在屏幕上的位置和设置窗口本身的大小。它的前两个参数是窗口在屏幕上的x和y坐标。后两个参数是窗口本身的宽和高
        ...
        self.setWindowTitle(u'Love You') # 仅仅设置窗体标题，不设置位置。
        self.setWindowIcon(QIcon('1.jpg')) # 调用QIcon构造函数时，我们需要提供要显示的图标的路径(相对或绝对路径)。同时注意：使用QIcon类型必须导入此模块from PyQt5.QtGui import QIcon
        ...
        self.label1.move(100,40) # 使此标签移动到这个部件(260,40)的位置
        ...
        self.window_pale.setBrush(self.backgroundRole(), QtGui.QBrush(QtGui.QPixmap("2.jpg")))#打开图片
        ...
        # Qt::NoFocus 0 不接受焦点
        self.buttonOK.move(50,550) # move()方法来指定部件的放置坐标，坐标的顶点就是窗口的左上角
        ...
        # Qt::WheelFocus StrongFocus | 0x4  滑轮作为焦点选中事件
        # Qt::NoFocus 0 不接受焦点
        self.buttonE.move(250, 550) # move()方法来指定部件的放置坐标，坐标的顶点就是窗口的左上角
        ...
        self.buttonNO.move(450,550)# move()方法来指定部件的放置坐标，坐标的顶点就是窗口的左上角
    ...
    def showDialogOK(self):
        QtWidgets.QMessageBox.information(self, "欧耶", "成功拿下！爱你么么哒", QtWidgets.QMessageBox.Ok)
        self.CloseAllowed = 1


    def showDialogEE(self):
        QtWidgets.QMessageBox.information(self, "别纠结了", "你妈让你做我女朋友", QtWidgets.QMessageBox.Ok)
        QtWidgets.QMessageBox.information(self, "别纠结了", "你爸也是这么说的",QtWidgets.QMessageBox.Ok)
        QtWidgets.QMessageBox.information(self, "别纠结了", "你奶奶也同意了", QtWidgets.QMessageBox.Ok)
    ...
    def showDialogNO(self):
        ...
        QtWidgets.QMessageBox.information(self, "不可能的", "你没机会拒绝我的", QtWidgets.QMessageBox.Ok)
